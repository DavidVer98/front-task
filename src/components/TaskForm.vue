<template>
  ...
  <form @submit.prevent="saveTask()"   v-if="loading" class="max-w-lg mx-auto my-10 bg-white p-8 rounded-xl shadow shadow-slate-300">

    <div v-if="create" class="flex p-4 mb-4 text-sm text-green-700 bg-green-100 rounded-lg dark:bg-green-200 dark:text-green-800 mb-5" role="alert">
    ...
    <div v-if="error" class="flex p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg dark:bg-red-200 dark:text-red-800 mb-5" role="alert">
        ...
        <span class="sr-only">Info</span>
        <div>
          <span class="font-medium">Danger alert!</span> Problem creating the task 😢
        </div>
    </div>

        <div class="flex flex-row justify-between items-center">
            <div>
                <h1 class="text-3xl font-medium">Create new task</h1>
            </div>
            <div class="inline-flex space-x-2 items-center">
                <button  class="p-2 border border-slate-200 rounded-md inline-flex space-x-1 items-center text-indigo-200 hover:text-white bg-indigo-600 hover:bg-indigo-500">
                  <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path></svg>
                      <span class="text-sm font-medium hidden md:block">Create</span>                     
                </button>
        ...
        <p class="text-slate-500">Hello, create here your new task</p>

        <div id="tasks" class="my-5">
            <hr>
            <div class="my-5">
              <label for="default-input" class="block mb-0 text-sm font-medium text-gray-900 dark:text-black">Write a task</label>
          ...
          <div class="my-5">
              <label for="default-input" class="block mb-0 text-sm font-medium text-gray-900 dark:text-black">Write a description</label>
  ...
  <!-- <form @submit.prevent="saveTask()">
    <input type="text" v-model="task.title" placeholder="Write a task" />
    <br />
    <textarea
...
import { Task } from "@/interfaces/Task"; // Interface Task is defined in mevn-typescript\src\interfaces\Task.ts
...
export default defineComponent({
  ...
  data() {
    return {
      tasks: [] as Task[], // Array of Task objects
      task: {} as Task, // task is an object of type Task
      loading : true,
      create: false,
      error : false
    };
  ...
  methods: {
    saveTask() {
      TaskService.createTask(this.task).then((res) => {
        this.tasks.push(res);
        this.task = {} as Task;
        this.loading = true
        this.create = true
